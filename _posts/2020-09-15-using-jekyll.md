...
So I'm using [Remarkbox](https://www.remarkbox.com/), also free (maximum of one domain) but with no ads. This is not covered by the theme, so we have to set it up and create the page our_evle_d. To set it up, you enter you site url and they'll give you a html code. Now on the folder `_includes` create a folder named `comments providers` and inside it, add a file called `custom.html` with the html code you just got from remarkbox.
